...
const unsigned long ERROR_FLASH_DURATION = 150;
const unsigned long LOOP_KEY_INTERVAL    = 1000;
const int           LOOP_KEY_MAX_ITER    = 300;   // max ~5 min at 1 s/iter
...
void sendSelectedKey() {
  ...
  switch (k.type) {
    ...
    case KEY_TYPE_LOOP:
      for (int i = 0; i < LOOP_KEY_MAX_ITER; i++) {
        bleKeyboard.print((char)k.charCode);
        delay(LOOP_KEY_INTERVAL);
        M5.update();
